...
import { PART_ONE_QUESTIONS, PART_TWO_QUESTIONS, PART_THREE_QUESTIONS } from './constants';
...
const App: React.FC = () => {
  ...
  const currentQuestions = useMemo(() => {
    switch(gameState.currentSection) {
      case GameSection.PART_ONE: return PART_ONE_QUESTIONS;
      case GameSection.PART_TWO: return PART_TWO_QUESTIONS;
      case GameSection.PART_THREE: return PART_THREE_QUESTIONS;
      default: return PART_ONE_QUESTIONS;
    }
  ...
  const renderIntro = () => (
    <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-10 animate-fade-in relative">
      ...
      <button onClick={startGame} className="group relative px-20 py-8 bg-orange-500 hover:bg-orange-600 text-white rounded-full text-4xl font-kids transition-all transform hover:scale-110 active:scale-95 shadow-[0_12px_0_rgb(194,65,12)]">
        開始挑戰！
  ...
  const renderGame = () => {
    ...
    const isPartThree = gameState.currentSection === GameSection.PART_THREE;
    ...
    return (
      <div className="flex flex-col h-full p-4 md:p-8 space-y-4">
        <div className="relative h-20 w-full flex items-center px-12 mt-4">
          ...
          <div className="absolute left-0 transition-all duration-1000 cubic-bezier(0.34, 1.56, 0.64, 1)" style={{ left: `calc(${progressPercent}% - 60px)` }}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-white/95 rounded-[4rem] p-8 shadow-[0_30px_80px_rgba(180,83,9,0.15)] border-[12px] border-white relative overflow-hidden bg-farm">
          ...
          <div className="mb-6 flex flex-col items-center space-y-3 relative z-10">
            <div className={`px-6 py-2 rounded-full font-kids text-xl shadow-sm border-2 ${
              gameState.currentSection === GameSection.PART_ONE ? 'bg-green-100 text-green-700 border-green-200' : 
              gameState.currentSection === GameSection.PART_TWO ? 'bg-blue-100 text-blue-700 border-blue-200' :
              'bg-purple-100 text-purple-700 border-purple-200'
            }`}>
              {gameState.currentSection === GameSection.PART_ONE ? '📍 漢字森林' : 
               gameState.currentSection === GameSection.PART_TWO ? '📍 拼音田野' :
               '📍 規則與對應'}
            </div>
            <h2 className="text-4xl font-kids text-stone-700 text-center max-w-2xl px-4">
              {isPartThree ? '請選出最正確的一項：' : (gameState.currentSection === GameSection.PART_ONE ? '選出對應的字：' : '選出正確的韻母：')}
            </h2>
          </div>

          <div className="flex-1 flex flex-col items-center justify-center w-full relative z-10">
            <div className={`text-center transition-all ${isPartThree ? 'text-4xl md:text-5xl py-12' : (gameState.currentSection === GameSection.PART_ONE ? 'text-9xl' : 'text-7xl md:text-8xl')} font-bold text-orange-600 bg-orange-50/50 px-12 py-10 rounded-[3rem] border-4 border-dashed border-orange-200 shadow-inner relative font-kids`}>
              {currentQuestion?.displayWord || currentQuestion?.target}
              <div className="absolute -top-6 -right-6 text-4xl animate-wiggle">💡</div>
            </div>

            <div className={`mt-10 grid gap-6 w-full max-w-3xl ${isPartThree ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-2'}`}>
              {currentQuestion?.options.map((opt, idx) => (
                <button
                  key={idx}
                  onClick={() => handleAnswer(opt)}
                  disabled={!!feedback}
                  className={`group relative py-6 md:py-8 ${isPartThree ? 'text-3xl' : 'text-5xl'} font-kids rounded-3xl shadow-xl border-b-[10px] transition-all active:scale-90 active:border-b-0 active:translate-y-[10px] ${
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <button onClick={() => switchSection(GameSection.PART_ONE)} className={`py-4 rounded-2xl font-kids text-xl transition-all border-4 flex flex-col items-center justify-center shadow-lg ${gameState.currentSection === GameSection.PART_ONE ? 'bg-orange-500 text-white border-orange-400 scale-95' : 'bg-white text-orange-600 border-orange-50 hover:bg-orange-50'}`}>
              <span className="text-2xl mb-1">🌲</span>漢字森林
            </button>
            <button onClick={() => switchSection(GameSection.PART_TWO)} className={`py-4 rounded-2xl font-kids text-xl transition-all border-4 flex flex-col items-center justify-center shadow-lg ${gameState.currentSection === GameSection.PART_TWO ? 'bg-orange-500 text-white border-orange-400 scale-95' : 'bg-white text-orange-600 border-orange-50 hover:bg-orange-50'}`}>
              <span className="text-2xl mb-1">🌽</span>拼音田野
            </button>
            <button onClick={() => switchSection(GameSection.PART_THREE)} className={`py-4 rounded-2xl font-kids text-xl transition-all border-4 flex flex-col items-center justify-center shadow-lg ${gameState.currentSection === GameSection.PART_THREE ? 'bg-orange-500 text-white border-orange-400 scale-95' : 'bg-white text-orange-600 border-orange-50 hover:bg-orange-50'}`}>
              <span className="text-2xl mb-1">📖</span>規則對應
            </button>
          </div>
          <div className="flex justify-center">
              <div className="bg-white/90 px-10 py-3 rounded-full border-4 border-orange-50 shadow-md flex items-center space-x-6">
                 ...
                 <span className="text-2xl font-kids text-stone-600">小兔籃子：<span className="text-4xl font-bold text-orange-500">{gameState.score}</span> 根</span>
              </div>
          </div>
        </div>
      </div>
  ...
  const renderSummary = () => (
    <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-12 animate-fade-in">
      ...
      <p className="text-3xl text-stone-600 leading-relaxed font-kids bg-white/60 p-10 rounded-[3rem] shadow-sm max-w-xl">
        有了你的幫忙，小兔挖到了好多胡蘿蔔！<br/>他是森林裡最幸福的小兔子！🥕✨
      </p>
      <div className="flex space-x-8">
        <button onClick={startGame} className="px-16 py-8 bg-green-500 hover:bg-green-600 text-white rounded-full text-4xl font-kids transition-all shadow-[0_12px_0_rgb(22,101,52)] active:translate-y-2 active:shadow-none">重新開始</button>
        <button onClick={() => switchSection(GameSection.PART_THREE)} className="px-16 py-8 bg-orange-500 hover:bg-orange-600 text-white rounded-full text-4xl font-kids transition-all shadow-[0_12px_0_rgb(194,65,12)] active:translate-y-2 active:shadow-none">挑戰規則</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen max-w-5xl mx-auto flex flex-col bg-farm relative selection:bg-orange-100">
      ...
      <main className="flex-1 overflow-hidden relative z-10">
        ...
        {(gameState.currentSection === GameSection.PART_ONE || gameState.currentSection === GameSection.PART_TWO || gameState.currentSection === GameSection.PART_THREE) && renderGame()}
